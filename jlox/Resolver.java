...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
    ...
    public Void visitThisExpr(Expr.This expr) {
        if (currentClass == ClassType.NONE) {
            Lox.error(expr.keyword, "Can't use 'this' outside of any class.");
            return null;
        }
    ...
    public Void visitSuperExpr(Expr.Super expr) {
        if (currentClass != ClassType.SUBCLASS) {
            Lox.error(expr.keyword, "Can't use 'super' outside of subclass.");
            return null;
        }
        resolveLocal(expr, expr.keyword);
        return null;
    ...
    private void resolveFunction(Stmt.Function function, FunctionType type) {
        resolveFunctionLike(function.params, function.body, type);
    }

    private void resolveFunction(Expr.Lambda lambda, FunctionType type) {
        resolveFunctionLike(lambda.params, lambda.body, type);
    }

    private void resolveFunctionLike(List<Token> params, List<Stmt> body, FunctionType type) {
